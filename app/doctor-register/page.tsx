...
const DoctorRegister = () => {
  ...
  const validate = () => {
    ...
    if (!formData.hospital_name) {
      err.hospital_name = "Hospital required";
    }

    // SLMC Registration Number
    const slmcPattern = /^[0-9]{4,10}$/;

    if (!formData.slmc_reg_no) {
      err.slmc_reg_no = "SLMC required";
    } else if (!slmcPattern.test(formData.slmc_reg_no)) {
      err.slmc_reg_no = "Invalid SLMC number";
    }
    ...
    if (!formData.profile_image) {
      err.profile_image = "Image required";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    Object.keys(formData).forEach((key) => {
      if (key !== "confirmPassword") {
        data.append(key, formData[key as keyof DoctorForm] as any);
      }
    });
  ...
  return (
    <div className="min-h-screen flex items-center text-gray-900 justify-center bg-[#f3e6f5] p-4">
      <div className="relative w-full max-w-5xl bg-gray-100  rounded-md p-6 md:p-10 overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-3">
            ...
            />
            {errors.slmc_reg_no && (
              <p className="text-red-500">{errors.slmc_reg_no}</p>
            )}
            ...
            <p className="text-center text-sm">
              ...
              <a className="text-blue-500" href="/doctor-login">
                Login
              </a>
            </p>
          </form>
