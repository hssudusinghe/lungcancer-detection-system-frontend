...
const Update = () => {
    ...
    const fetchDoctor = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/doctor/get-by-id/${userId}`,
        );
  ...
  const validate = () => {
    ...
    if (!formData.slmc_reg_no) {
      err.slmc_reg_no = "Please enter SLMC registration number";
    } else if (!slmcPattern.test(formData.slmc_reg_no)) {
      err.slmc_reg_no = "SLMC number should contain 4 to 10 digits";
    }
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/doctor/update/${userId}`,
        {
          method: "PUT",
          body: data,
        },
